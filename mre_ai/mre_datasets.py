...
from mre_ai.registration import RegPatient, Register
...
class MREtoXr:
    ...
    def __init__(self, data_dir=None, sequences=None, patient=None, from_file=None, **kwargs):
        ...
        # NEWEST DEEPLAB VERSION
        if self.mask_arch == 'DeepLab':
            model_path = Path('/ocean/projects/asc170022p/bpollack/mre_ai/data/CHAOS/',
    ...
    def load_xr(self):
        ...
        resized_t1_pre, _ = self.reg_inputs(reg_pat)
...
def mre_to_pyviz_ds(path_to_subj):
    '''convert an mre ds to a pyviz generic ds'''

    ds_mre = xr.open_dataset(path_to_subj)['image_mri']
    ds_img_shape = ds_mre.transpose('subject', 'sequence', 'z_mri', 'y', 'x').shape
    # ds_ovrly_shape = (ds_img_shape[0], ds_img_shape[1], 1, ds_img_shape[2], ds_img_shape[3],
    #                   ds_img_shape[4])

    ds = xr.Dataset({'image': (['subject_id', 'label', 'z', 'y', 'x'],
                               ds_mre.transpose('subject', 'sequence', 'z_mri', 'y', 'x').values),
                     # 'overlay': (['subject_id', 'label', 'overlay_label', 'z', 'y', 'x'],
                     #             np.zeros(ds_ovrly_shape, dtype=int)),
                     'spacing': (['subject_id', 'label', 'img_dims'],
                                 np.ones((ds_img_shape[0], ds_img_shape[1], 3))),
                     'origin': (['subject_id', 'label', 'img_dims'],
                                np.zeros((ds_img_shape[0], ds_img_shape[1], 3))),
                     },
                    coords={'subject_id': ds_mre.subject.values,
                            'label': ds_mre.sequence.values,
                            # 'overlay_label': ['seg_0'],
                            'z': ds_mre.z_mri.values,
                            'y': ds_mre.y,
                            'x': ds_mre.x,
                            'img_dims': range(3),
                            }
...
def mre_wave_to_pyviz_ds(path_to_subj):
    '''convert an mre ds to a pyviz generic ds'''

    ds_in = xr.open_dataset(path_to_subj)
    ds_mre = ds_in['image_mre'].sel(mre_type=['mre', 'mre_wave'])
    ds_mask = ds_in['mask_mre'].sel(mask_type=['mre'])
    ds_img_shape = ds_mre.transpose('subject', 'mre_type', 'z_mre', 'y', 'x').shape
    ds_ovrly_shape = (ds_img_shape[0], ds_img_shape[1], 1, ds_img_shape[2], ds_img_shape[3],
                      ds_img_shape[4])

    ds = xr.Dataset({'image': (['subject_id', 'label', 'z', 'y', 'x'],
                               ds_mre.transpose('subject', 'mre_type', 'z_mre', 'y', 'x').values),
                     'overlay': (['subject_id', 'label', 'overlay_label', 'z', 'y', 'x'],
                                 np.zeros(ds_ovrly_shape, dtype=int)),
                     'spacing': (['subject_id', 'label', 'img_dims'],
                                 np.ones((ds_img_shape[0], ds_img_shape[1], 3))),
                    ...
                    coords={'subject_id': ds_mre.subject.values,
                            'label': ds_mre.mre_type.values,
                            'overlay_label': ['mre_mask'],
                            'z': ds_mre.z_mre.values,
                            'y': ds_mre.y,
                            'x': ds_mre.x,
                            'img_dims': range(3),
                            }
                    )
    ds['overlay'].loc[dict(label='mre')] = ds_mask.transpose(
        'subject', 'mask_type', 'z_mre', 'y', 'x').values
    ds['overlay'].loc[dict(label='mre_wave')] = ds_mask.transpose(
        'subject', 'mask_type', 'z_mre', 'y', 'x').values
    return ds
...
class MRETorchDataset(Dataset):
    ...
    def get_data_aug_3d(self, image, target, mask):
        ...
        for i in range(image.shape[0]):
            ...
            for j in range(image.shape[1]):
                slice_list.append(self.affine_transform(
                    image[i][j], rot_angle_xy, translations_xy, scale,
        ...
        for j in range(mask.shape[1]):
            mask_list.append(self.affine_transform(
                mask[0][j], rot_angle_xy, translations_xy, scale,
                interpolation=TF.InterpolationMode.NEAREST, erode_mask=self.erode_mask))
    ...
    def affine_transform(self, input_slice, rot_angle=0, translations=0, scale=1, interpolation=None,
                         erode_mask=0):
        if erode_mask != 0:
        ...
        input_slice = TF.affine(input_slice, angle=rot_angle,
                                ...
                                interpolation=interpolation, fill=outer_pixel_val)
...
def clinical_df_maker():
    df_clinical = pd.read_excel(
        '/ocean/projects/asc170022p/shared/Data/MRE/MRE_dataset_clinical.xlsx', engine='openpyxl')
